...
class GMixGalsim(object):
    ...
    def get_run_dir(self):
        dir=os.path.expanduser('~/galsim/%s' % self.run)
        return dir
    def get_image_dir(self):
        dir=self.get_run_dir()
        dir=os.path.join(dir, self.profile)
        return dir
    def get_image_basename(self):
        bname='ellip%02ihlr%02i.fits' % (self.ellip,self.res)
    def get_image_file(self):
        dir=self.get_image_dir()
        f=os.path.join(dir,)
    def get_psf_file(self):
        dir=self.get_image_dir()
        f=os.path.join(dir,'psf.fits')

    def get_output_dir(self):
        dir=os.path.expanduser('~/galsim-outputs/%s/%s' % (self.run,self.profile))
        return dir
